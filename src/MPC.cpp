...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    double epsi_weight = 5000;
    double v_weight = 1;
    double delta_weight = 8;
    double a_weight = 8;
    double delta_smooth_weight = 100;
    double a_smooth_weight = 5;

    // cost cte, psi error, and velocity
    for (unsigned int i=0; i < N; i++) {
      fg[0] += cte_weight * CppAD::pow(vars[cte_start+i], 2);
      fg[0] += epsi_weight * CppAD::pow(vars[epsi_start+i], 2);
      fg[0] += v_weight * CppAD::pow(vars[v_start+i] - v_guide, 2);
    }

    // cost steering angle and acceleration
    for (unsigned int i=0; i < N-1; i++) {
      fg[0] += delta_weight * CppAD::pow(vars[delta_start+i], 2);
      fg[0] += a_weight * CppAD::pow(vars[a_start+i], 2);
    }

    // add some temporal smoothness for steering angle and acceleration
    for (unsigned int i=0; i < N-2; i++) {
    fg[0] += delta_smooth_weight * CppAD::pow(vars[delta_start+i+1] - vars[delta_start+i], 2);
    fg[0] += a_smooth_weight * CppAD::pow(vars[a_start+i+1] - vars[a_start+i], 2);
    }

    // initialize the model to the initial state
    // recall fg[0] is reserved for the cost value, so the other indices increased by 1
